...
The semiconductor industry provides the closest parallel — and the warning. In the 1980s, Japanese semiconductor firms, backed by government industrial policy through MITI, sold DRAM chips below production cost in American and European markets. Within a decade, the American DRAM industry was effectively destroyed. Intel survived only by pivoting to microprocessors. The United States responded with tariffs, trade agreements, and ultimately a decades-long industrial policy — culminating in the CHIPS Act of 2022 — to rebuild domestic semiconductor manufacturing capacity. The lesson was seared into trade policy: when a strategic technology is given away below cost, the short-term benefit to consumers masks a long-term transfer of capability and dependency.

AI models are not semiconductors. They can be copied for free, they improve on different curves, and the "manufacturing" is training, not fabrication. But the structural logic holds. When Alibaba releases Qwen freely, it is seeding architectural dependency in the same way that underpriced DRAM seeded hardware dependency. Every developer who fine-tunes a Qwen derivative inherits Alibaba's tokenizer, Alibaba's architectural assumptions, and — when the fine-tuned model hits its limits — a natural upgrade path to Alibaba's paid cloud inference. The dependency is soft, not hard. It can be escaped. But escaping it means retraining, reintegrating, retesting — costs that most developers will not bear when the next version of Qwen is free and the migration is seamless.
